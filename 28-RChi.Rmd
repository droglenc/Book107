...
In Modules \@ref(ChiSquare) and \@ref(GOFTest) you learned the theory and how to perform the calculations required to complete the 11 steps (Section \@ref(Hyp11Steps)) of a hypothesis test for chi-square and goodness-of-fit tests. In this module, you will learn how to perform the required calculations from raw data using R.
...
The Chi-Square Test is performed with `chisq.test()`, which takes the observed frequency table  as the first argument and `correct=FALSE` so that the continuity correction is not used.^[The continuity correction is not used here simply so that the results using R will match hand-calculations. The continuity correction should usually be used.] The results of `chisq.test()` should be assigned to an object. The Chi-Square test statistic and p-value are extracted by simply printing the saved object. The expected frequency table is returned by appending `$expected` to the saved object.
...
As with the Chi-Square Test, the Goodness-of-Fit test can be made from either raw data, that are then summarized, or data that has already been summarized. Raw data are summarized into a one-way observed frequency table using `xtabs()` with a formula of `~cvarRow` as described in Section \@ref(frequency-and-percentage-tables). The results should be saved to an object for later use.
