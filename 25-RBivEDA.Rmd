...
::: {.tip data-latex=''}
When making your own scatterplot, copy the code above and change the items in `data=`, `x=`, `y=`, and `x=`, and `y=`. All other items can remain as shown above.
:::

### Correlation Coefficient
The correlation coefficient (r) between two quantitative variables is computed with `corr()` using a formula of the form `~qvarY+qvarX`,^[Note that `qvarY~qvarX` will also work.] where `qvarY` and `qvarX` are the names of quantitative variables, as the first argument and the corresponding data frame in `data=`.^[`corr()` is from `NCStats`.] The number of decimal places is controlled with `digits=`.^[Two or three decimals should be used for correlation coefficients.] For example, the correlation coefficient between highway MPG and weight for all cars in the car data is `r formatC(corr(~HMPG+Weight,data=cars93),format="f",digits=3)`.
