...
## Confidence Concept
An understanding of what it means to be "95% confident" requires examination of multiple samples from a population, as was done in Module \@ref(SamplingDist) when considering sampling variability. In this initial discussion, only 95% confidence intervals (CI), where a range (i.e., bounded on both ends) is computed, are considered. These simplifying restrictions and unrealistically knowing population values are used here only so that the **concept** of confidence intervals can be more easily explained. General methods for constructing other types of confidence regions with other levels of confidence are in Section \@ref(constructing-confidence-regions).

In the Square Lake example (introduced in Module \@ref(WhyStats)), it was known that $\mu$=98.06 and $\sigma$=31.49 (Table \@ref(tab:SquareLakePopnTbl)). Additionally, $\bar{x}$=100.04 was obtained from the first sample of n=50 (Table \@ref(tab:SquareLakeSample1s)). A 95% CI for $\mu$ is defined as $\bar{x}$&plusmn;2$SE_{\bar{x}}$. The 95% CI for the mean total length for the Square Lake population, computed from this one sample, is 100.04&plusmn;2$\frac{31.49}{\sqrt{50}}$, 100.04&plusmn;8.91, or (91.13,108.95). This interval contains $\mu$ (i.e., 98.06 is between 91.13 and 108.95). In other words, this particular CI accomplished what it was intended to do; i.e., provide a range that contains $\mu$.
...
* A CI is a random variable like any other statistic. That is, each sample results in a different 95% CI (see CI lines in Figures \@ref(fig:CIex100) and \@ref(fig:CI9080Ex)) just like it results in a different $\bar{x}$ (see dots on CI lines in Figures \@ref(fig:CIex100) and \@ref(fig:CI9080Ex)).
* Any CI either contains the parameter (e.g., $\mu$) or not. However, on average, 95% of 95% CIs will contain the parameter and 5% will not. That is, 95% of all possible 95% CIs wil contain the parameter.
...
tibble("$\\alpha$"=c(0.01,0.05,0.10),
       C=c("99%","95%","90%")) %>%
  knitr::kable(booktabs=TRUE,align="c",caption="Several common confidence levels (C) and the corresponding probability of a Type I error ($\\alpha$).") %>%
...
* Identify the level of confidence (i.e., C=100(1-$\alpha$)%; Table \@ref(tab:Calpha)).
* Identify the type of confidence regions -- interval, lower bound, or upper bound (Table \@ref(tab:HAsCI)).
* Determine the scaling factor.
* Compute the actual confidence region.
* Interpret the confidence region.

Consider the Square Lake example where H~A~:$\mu$<105, $\alpha$=0.05, and $\bar{x}$=100.04 from n=50 (Table \@ref(tab:SquareLakeSample1s)).

* C=95% (=100(1-0.05)).
* Upper confidence bound because H~A~ is a "less than."
* $Z^{*}$=+`r formatC(qnorm(0.95),format="f",digits=3)` as found with [Note that `mean=0` and `sd=1` are the default settings for `distrib()` and can, thus, be omitted when finding a $Z^{*}$.]
```{r fig.show='hide'}
( distrib(0.95,type="q") )
```
* 100.04+1.645$\frac{31.49}{\sqrt{50}}$, 100.04+7.33, or 107.37.
* One is 95% confident that the mean total length of ALL fish in Square Lake is less than 107.4 mm. By confident, it is meant that 95% of all 95% confidence regions will contain $\mu$.

Second, suppose that the Lake Superior ice cover data from Table \@ref(tab:LSIStats) (note that $\bar{x}$=107.8 and n=42) was tested with H~A~: $\mu \neq$ 100, $\sigma$=22, and $\alpha$=0.01.

* C=99% (=100(1-0.01)).
* Confidence interval because H~A~ is a "not equals."
* $Z^{*}$=&plusmn;`r formatC(qnorm(0.995),format="f",digits=3)` as found with
```{r fig.show='hide'}
( distrib(0.995,type="q") )
```
* 107.8&plusmn;2.576$\frac{22}{\sqrt{42}}$, 107.8&plusmn;8.74, or (99.06,116.54).
* One is 95% confident that the mean annual number of days of ice cover on Lake Superior is between 99.1 and 116.5 days. By confident, it is meant that 95% of all 95% confidence regions will contain $\mu$.

Finally, suppose that the second example hypothesis test in Module \@ref(HypTesting1) about battery life (i.e., H~A~:$\mu$>36 vs H~0~:$\mu$=36) is being tested with $\alpha$=0.10. Further suppose that $\sigma$=7 and that $\bar{x}$=45 from n=40.

* C=90% (=100(1-0.10)).
* Lower confidence bound because H~A~ is a "greater than."
* $Z^{*}$=`r formatC(qnorm(0.10),format="f",digits=3)` as found with
```{r fig.show='hide'}
( distrib(0.90,type="q",lower.tail=FALSE) )
```
* 45-1.282$\frac{7}{\sqrt{40}}$, 45-1.42, or 43.58.
* One is 90% confident that the mean life for ALL batteries with the additive is more than 43.58 months. By confident, it is meant that 95% of all 95% confidence regions will contain $\mu$.
