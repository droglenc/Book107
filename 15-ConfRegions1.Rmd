...
In the Square Lake example (introduced in Module \@ref(WhyStats)), it was known that &mu;=98.06 and &sigma;=31.49 (Table \@ref(tab:SquareLakePopnTbl)). Additionally, $\bar{\text{x}}$=100.04 was obtained from the first sample of n=50 (Table \@ref(tab:SquareLakeSample1s)). A 95% CI for &mu; is defined as $\bar{\text{x}}$&plusmn;2$SE_{\bar{\text{x}}}$. The 95% CI for the mean total length for the Square Lake population, computed from this one sample, is 100.04&plusmn;2$\frac{31.49}{\sqrt{50}}$, 100.04&plusmn;8.91, or (91.13,108.95). This interval contains &mu; (i.e., 98.06 is between 91.13 and 108.95). In other words, this particular CI accomplished what it was intended to do; i.e., provide a range that contains &mu;.
...
* A CI is a random variable like any other statistic. That is, each sample results in a different 95% CI (see CI lines in Figures \@ref(fig:CIex100) and \@ref(fig:CI9080Ex)) just like it results in a different $\bar{\text{x}}$ (see dots on CI lines in Figures \@ref(fig:CIex100) and \@ref(fig:CI9080Ex)).
...
Second, suppose that the Lake Superior ice cover data from Table \@ref(tab:LSIStats) (note that $\bar{\text{x}}$=107.8 and n=42) was tested with H~A~: &mu;&ne;100, &sigma;=22, and &alpha;=0.01.
...
Finally, suppose that the second example hypothesis test in Module \@ref(HypTesting1) about battery life (i.e., H~A~:&mu;>36 vs H~0~:&mu;=36) is being tested with &alpha;=0.10. Further suppose that &sigma;=7 and that $\bar{\text{x}}$=45 from n=40.
