...
1. The assumptions are met because ...
    i. n=50&geq;40 and
    i. $\sigma$ is unknown.
1. $\bar{x}$ = 20.1 lbs (and s = 0.76 lbs).
...
1. $\alpha$=0.01.
1. H~0~:$\mu$=98.6^o^F vs. H~A~:$\mu$$\neq$98.6^o^F, where $\mu$ is the mean body temperature for ALL healthy humans. [*Note that not equals was used because the researchers want to determine if the temperature is different from $98.6$^o^F.*]
1. A 1-Sample t-Test is required because ...
...
1. The assumptions are met because ...
    ...
    i. $\sigma$ is unknown.
1. $\bar{x}$ = `r formatC(sum[["mean"]],format="f",digits=2)`^o^F.
...
1. It appears that the mean body temperature of ALL healthy humans is less than 98.6^o^F. [*Note that the test was for a difference but because $\bar{x}$<98.6 this more specific conclusion can be made.*]
