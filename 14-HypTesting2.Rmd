...
In addition, &beta; increases as the difference between the hypothesized mean (&mu;~0~) and the actual mean (&mu;~A~) decreases (Figure \@ref(fig:SLBetaRelations)). This means that you will make more Type II errors when the hypothesized and actual mean are close together. In addition, &beta; increases with a increasing amount of natural variability (i.e., &sigma;; Figure \@ref(fig:SLBetaRelations)). In other words, more Type II errors are made when there is more variability among individuals. A researcher cannot control the difference between &mu;~0~ and &mu;~A~ or the value of &sigma;. However, it is important to know that if a situation with a "large" amount of variability is encountered or the difference to be detected is small, the researcher will need to increase n to reduce &beta;. For example, if n could be doubled in the Square Lake example to 100, then &beta; (for H~0~:&mu;=105) would decrease to approximately 0.18 (Figure \@ref(fig:SLBetaRelations)).
