...
In a 2-Sample t-Test, H~0~:$\mu_{1}=\mu_{2}$ states that the two population means are equal. This can be rewritten as H~0~:$\mu_{1}-\mu_{2}$=0, because the difference between two population means is zero if the two population means are equal. With this H~0~, the "parameter" is $\mu_{1}-\mu_{2}$ and the corresponding statistic is $\bar{x}_{1}-\bar{x}_{2}$. Thus, a 2-Sample t-Test is focused on the difference in population means.
...
it is apparent that the SE of $\bar{x}_{1}-\bar{x}_{2}$ (i.e., the statistic) is needed. Unfortunately, the calculation of this standard error depends on whether the two population variances are equal or not. When the variances are approximately equal (discussed in Section \@ref(LevenesTest)), the standard error of $\bar{x}_{1}-\bar{x}_{2}$ is
...
* **Hypothesis:** H~0~:$\mu_{1}-\mu_{2}$=0
* **Statistic:** $\bar{x}_{1}-\bar{x}_{2}$
* **Test Statistic:** $t=\frac{\bar{x}_{1}-\bar{x}_{2}-0}{\sqrt{s_{p}^{2}\left(\frac{1}{n_{1}}+\frac{1}{n_{2}} \right)}}$ where $s_{p}^{2}=\frac{(n_{1}-1)s_{1}^{2}+(n_{2}-1)s_{2}^{2}}{n_{1}+n_{2}-2}$.
* **Confidence Region:** $(\bar{x}_{1}-\bar{x}_{2})+t^{*}\sqrt{s_{p}^{2}\left(\frac{1}{n_{1}}+\frac{1}{n_{2}} \right)}$
* **df:** $n_{1}+n_{2}-2$
* **Assumptions:**
    * $n_{1}+n_{2}$&geq;40, $n_{1}+n_{2}$&geq;15 and **each sample** (i.e., histogram) is not strongly skewed, OR **each sample** is normally distributed.
    * Variances are equal,
...
1. $\alpha$ = 0.05.
1. H~0~:$\mu_{s}-\mu_{o}$ = 0 vs H~A~:$\mu_{s}-\mu_{o}$ > 0, where $\mu$ is the mean plant height, s represents synthetic fertilizer, and o represents organic fertilizer. [*Note that positive differences represent larger values for synthetic fertilizer; thus, H~A~ represents synthetic fertilizer producing taller plants.*]
...
1. $\bar{x}_{s}-\bar{x}_{0}$ = 51.46 - 47.49 = 3.97. Additionally, $s_{p}^{2}=\frac{(25-1)5.975^{2}+(25-1)6.721^{2}}{25+25-2}$ = 40.44 and $SE_{\bar{x}_{s}-\bar{x}_{o}}=\sqrt{40.44\left(\frac{1}{25}+\frac{1}{25} \right)}$ = 1.799.
...
1. $\alpha$ = 0.05.
1. H~0~:$\mu_{w}-\mu_{wo}$=0 vs H~A~:$\mu_{w}-\mu_{wo}$<0, where $\mu$ is the mean anxiety level, w represents patients "with", and wo represents "without" music. [*Note that negative numbers represent lower anxiety values in patients in the "with music" treatment. Thus, H~A~ suggests lower anxiety in patients with music.*]
1. A 2-Sample t-Test is required because ...
...
1. $\bar{x}_{w}-\bar{x}_{wo}$ = 4.2-5.9 = -1.7. Additionally, $s_{p}^{2}=\frac{(32-1)1.2^{2}+(36-1)1.9^{2}}{32+36-2}$ = 2.59 and $SE_{\bar{x}_{w}-\bar{x}_{wo}}=\sqrt{2.59\left(\frac{1}{32}+\frac{1}{36} \right)}$ = 0.391.
1. t = $\frac{-1.7-0}{0.391}$ = -4.348 with 32+36-2 = 66 df.
1. p-value = `r kPvalue(pt(-4.348,df=66),include.p=FALSE,latex=FALSE)`.
1. H~0~ is rejected because the p-value<$\alpha$.
1. The mean anxiety level appears to be lower when music was played for the patients.
